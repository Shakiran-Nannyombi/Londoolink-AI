...
from app.schemas.message import EmailMessage, CalendarEvent, GenericMessage, SocialMessage, WhatsAppMessage, InstagramMessage, TelegramMessage
...
async def ingest_calendar(
    ...
    try:
        logger.info(f"Ingesting calendar event for user {current_user.id}: {calendar_data.title}")
...
Attendees: {', '.join(calendar_data.attendees or [])}
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Calendar ingestion failed: {str(e)}"
        )


@router.post("/generic")
async def ingest_generic_message(
    message_data: GenericMessage,
    current_user: User = Depends(get_current_user)
) -> Dict[str, Any]:
    """Ingest generic message/document data."""
    try:
        logger.info(f"Ingesting generic message for user {current_user.id} from {message_data.source}")
        ...
        logger.error(f"Generic message ingestion failed for user {current_user.id}: {e}")
        raise HTTPException(
            status_code=500,
            detail=f"Generic message ingestion failed: {str(e)}"
        )
...
async def ingest_whatsapp_message(
    ...
    try:
        logger.info(f"Ingesting WhatsApp message for user {current_user.id} from {message_data.sender}")
...
Message: {message_data.content}
        ...
        raise HTTPException(
            status_code=500,
            detail=f"WhatsApp ingestion failed: {str(e)}"
        )


@router.post("/instagram")
async def ingest_instagram_message(
    message_data: InstagramMessage,
    current_user: User = Depends(get_current_user)
) -> Dict[str, Any]:
    """Ingest Instagram message data."""
    try:
        logger.info(f"Ingesting Instagram message for user {current_user.id} from {message_data.username}")
...
async def ingest_telegram_message(
    ...
    try:
        logger.info(f"Ingesting Telegram message for user {current_user.id} from {message_data.sender}")
...
Message: {message_data.content}
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Telegram ingestion failed: {str(e)}"
        )


@router.post("/social")
async def ingest_social_message(
    message_data: SocialMessage,
    current_user: User = Depends(get_current_user)
) -> Dict[str, Any]:
    """Ingest generic social media message data."""
    try:
        logger.info(f"Ingesting {message_data.platform} message for user {current_user.id} from {message_data.sender}")
...
Message: {message_data.content}
        ...
        logger.error(f"{message_data.platform} ingestion failed for user {current_user.id}: {e}")
        raise HTTPException(
            status_code=500,
