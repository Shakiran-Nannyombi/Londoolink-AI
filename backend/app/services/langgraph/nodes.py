...
class AgentNodes:
    ...
    def coordinator_node(self, state: AgentState) -> AgentState:
        ...
        logger.info(
            f"Coordinator processing step: {state.get('current_step', 'start')}"
        )
        ...
        if current_step == "start":
            ...
            state["messages"] = [
                HumanMessage(
                    content=f"Generate daily briefing for user {state['user_id']}"
                )
            ]
    ...
    def priority_agent_node(self, state: AgentState) -> AgentState:
        ...
        try:
            # Gather all analyses
            email_analysis = state.get("email_analysis", {}).get(
                "analysis", "No email analysis"
            )
            calendar_analysis = state.get("calendar_analysis", {}).get(
                "analysis", "No calendar analysis"
            )
