...
class SecurityValidator:
    ...
    def _validate_jwt_config(self):
        ...
        if not hasattr(settings, 'ACCESS_TOKEN_EXPIRE_MINUTES'):
            ...
            self.warnings.append("ACCESS_TOKEN_EXPIRE_MINUTES is very long (>24h), consider shorter expiration")
        elif settings.ACCESS_TOKEN_EXPIRE_MINUTES < 5:
            self.warnings.append("ACCESS_TOKEN_EXPIRE_MINUTES is very short (<5min), may cause UX issues")
    ...
    def _validate_encryption_config(self):
        ...
        if not hasattr(settings, 'ENCRYPTION_KEY') or not settings.ENCRYPTION_KEY:
            ...
            self.issues.append("ENCRYPTION_KEY must be at least 32 bytes (64 hex characters)")
    ...
    def _validate_database_config(self):
        ...
        elif "password" in settings.DATABASE_URL.lower() and settings.DATABASE_URL.count("password") > 1:
    ...
    def _validate_production_config(self):
        ...
        if hasattr(settings, 'DATABASE_URL') and settings.DATABASE_URL.startswith('postgresql://'):
            self.warnings.append("Consider using postgresql+psycopg2:// for better performance")
    ...
    def get_recommendations(self) -> List[str]:
        ...
        if any("ENCRYPTION_KEY" in issue for issue in self.issues):
            recommendations.append("Generate a new ENCRYPTION_KEY using: python -c \"import os; print(os.urandom(32).hex())\"")
        
        if any("default" in issue.lower() for issue in self.issues):
            recommendations.append("Replace all default credentials and keys with secure values")
        
        if any("production" in warning.lower() for warning in self.warnings):
            recommendations.append("Review production security settings and enable HTTPS")
